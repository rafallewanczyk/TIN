...
public class RegulatorCommunicator {
    ...
    public void sendTemperatureChangeConfig(Regulator regulator, List<TemperatureDevice> devices) throws IOException, BadPaddingException, InvalidKeyException, IllegalBlockSizeException, InvalidKeySpecException, NoSuchAlgorithmException, InvalidAlgorithmParameterException, SignatureException {
        DataWithSignature response = communicator.sendData(
                ...
                regulator.getPort(),
                TIMEOUT,
                KeyEncoder.getPublicKey(regulator.getPublicKey()),
    ...
    private byte[] temperatureDevicesToByteArray(List<TemperatureDevice> temperatureDevices) throws IOException, InvalidKeySpecException, NoSuchAlgorithmException {
    ...
    private byte[] lightDevicesToByteArray(List<LightDevice> lightDevices) throws IOException, InvalidKeySpecException, NoSuchAlgorithmException {
        ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
        DataOutputStream dataOutputStream = new DataOutputStream(byteStream);
        dataOutputStream.write("CHANGE_CONFIG".getBytes());
    ...
    private void writeDeviceBasicAttributes(DataOutputStream dataOutputStream, Device device) throws IOException, InvalidKeySpecException, NoSuchAlgorithmException {
        ...
        dataOutputStream.writeInt(KeyEncoder.getPublicKey(device.getPublicKey()).getEncoded().length);
        dataOutputStream.write(KeyEncoder.getPublicKey(device.getPublicKey()).getEncoded());
    }
